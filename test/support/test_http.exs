defmodule Support.TestHttp do
  def get(url, _) do
    cond do
      String.contains?(url, ["rubyconf2015", "PLSD48HvrE6"]) ->
        {:ok, %{status_code: 200, body: "{}"}}
      String.contains?(url, "rubyconf2014") ->
        {:ok, %{status_code: 200, body: fixture(:confreaks_events_json)}}
      String.contains?(url, "PLSD48HvrE7") ->
        {:ok, %{status_code: 200, body: fixture(:youtube_events_json)}}
      true ->
        {:error, %{status_code: 400, body: "{}"}}
    end
  end

  defp fixture(:youtube_events_json) do
    ~s(
{
 ...
 "items": [
  ...
  defp fixture(:confreaks_events_json) do
    ~s(
      [
        {
          "id": 4825,
